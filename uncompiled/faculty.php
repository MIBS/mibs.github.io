<?php include 'header.php'; ?>

<div class="container-fluid">
	<section class="row first-section" id="faculty">

	<div class="col-md-8 col-md-offset-2">

		<h2>
			Faculty	
		</h2>

		<hr />

		<div class="person-card">

			<img src="./images/pictures/jackie.png" class="img-circle" style="height:100px; width:100px"> 
			<h4>Jackie Vogel</h4>
			<p>
				Head of the Quantitative Biology Program
			</p>
		</div>
		<div class="person-card">

			<img src="./images/pictures/nancy.jpg" class="img-circle" style="height:100px; width:100px">

			<h4>Nancy Nelson</h4>
			<p>
				Biology Departmental Advisor
			</p>
		</div>
		<div class="person-card">

			<img src="./images/pictures/blanchette.jpg" class="img-circle" style="height:100px; width:100px">
			<h4>Mathieu Blanchette</h4>
			<p>
				Joint Computer Science and Biology Program Coordinator
			</p>
		</div>
		<div class="person-card">

			<img src="./images/pictures/fred.jpg" class="img-circle" style="height:100px; width:100px">
			<h4>Frederic Guichard</h4>
			<p>
				Joint Biology and Mathematics Program Coordinator
			</p>
		</div>

	</div>
<!-- @TODO: make these into boxes which automatically resize and rearrage as necessary -->
		<!-- <div class="container-fluid">
			<div class="row">

			<div class="col-sm-1 col-sm-offset-3 col-md-1 col-md-offset-3">
				JACKIE VOGEL
			</div>

			<div class="col-sm-1 col-md-1">
				JACKIE VOGEL
			</div>

			<div class="col-sm-1 col-md-1">
				JACKIE VOGEL
			</div>
		</div> -->

	</section>
</div>

<?php include 'footer.php'; ?>